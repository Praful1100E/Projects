...
# Import the specific Twilio exception for better error handling
...
def send_alert(message):
    ...
    if not all([account_sid, auth_token, twilio_phone_number, owner_phone_number]):
        ...
        if not account_sid: print("DEBUG: TWILIO_ACCOUNT_SID is missing.")
        if not auth_token: print("DEBUG: TWILIO_AUTH_TOKEN is missing.")
        if not twilio_phone_number: print("DEBUG: TWILIO_PHONE_NUMBER is missing.")
        if not owner_phone_number: print("DEBUG: OWNER_PHONE_NUMBER is missing.")
        return
    ...
    try:
        ...
        # Catch specific Twilio errors for better debugging
...
def get_weather_forecast(location="Bhoranj,IN"):
    ...
    if not API_KEY: return {"error": "Weather API Key not set."}
...
@app.route('/')
def index():
    """Renders the main dashboard page, now with initial AI analysis."""
