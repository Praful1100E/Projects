...
def numbers_only_bruteforce():
    """Brute-force: numbers only, up to 4 digits."""
    global stop_flag
    if not selected_pdf:
    ...
    max_length = 4  # change to 5 if needed
    stop_flag = False
    lbl_status.config(text="Brute-forcing numbers only...", fg="blue")
...
tk.Button(root, text="Start Numbers-Only Brute-Force", command=numbers_only_bruteforce, width=25, bg="red", fg="white").pack(pady=5)
tk.Button(root, text="Stop", command=stop_cracking, width=25, bg="orange").pack(pady=5)
