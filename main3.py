...
def send_alert(message):
    ...
    twilio_phone_number = os.environ.get(8278853082)
    owner_phone_number = os.environ.get(6230951985)

    if not all([account_sid, auth_token, twilio_phone_number, owner_phone_number]):
        print("Twilio environment variables not set. Skipping SMS.")
        return
...
def get_weather_forecast(city="Himachal Pradesh,IN"):
    """Fetches weather data from OpenWeatherMap for the specified city."""
    # This is where your API key is used from environment variables
    API_KEY = os.environ.get('OPENWEATHER_API_KEY')
    if not API_KEY:
        return {"error": "Weather API Key not set."}
    
    # This is the URL structure you provided
    ...
    try:
        ...
        return {"error": f"HTTP error fetching weather: {http_err}"}
...
def index():
    ...
    # The weather function is now called with "Himachal Pradesh" as the default
